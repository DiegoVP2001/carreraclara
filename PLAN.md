...
| **25** | Auditoría de datos no considerados por error | `mifuturo/`, `web/*.html`, `web/export_*.py` | ✅ 2026-08-15 — El hueco de T24 no era aislado de NEM: "Carreras por institución" (`instituciones_v2.html`) hereda el mismo problema porque se construye solo desde `hecho_indicadores` (grano título) — **1183 combos institución+carrera con oferta real quedan invisibles ahí también**, confirmado por SQL (mismo número exacto que T24). Además, 31 columnas cargadas en `comparador.db` con datos reales nunca se consultan (costo titulación, duración formal, matrícula/titulación por sexo, series históricas 2020-2023 y percentiles de ingreso). Informe completo en `AUDITORIA_DATOS_T25.md`. Sin fixes en esta sesión (tarea de diagnóstico). |
| **25b** | Corregir cobertura en "Carreras por institución" (hallazgo T25-1) | `GUIA_FUENTES_DE_DATOS.md`, `web/instituciones_v2.html` | ✅ 2026-08-15 — Decisión (discutida con Diego): **no** se agregan los 1183 combos institución+genérica solo-`hecho_oferta` a "Carreras por institución" — esa tabla no tiene, ni tendrá, empleabilidad/ingreso/retención, y el mismo pool ya está cubierto por `nem.html` (T24). Documentado en `GUIA_FUENTES_DE_DATOS.md` (nueva guía de fuentes de datos) en vez de tocar el export; `aria-label` desactualizado corregido en `instituciones_v2.html`. |
| **26** | Brainstorm: qué construir con toda la data disponible | ninguno (sesión de ideación, sin código) | ✅ 2026-08-15 — `BRAINSTORM_T26.md`: ideas desde datos T25 (31 columnas cargadas/no usadas) + investigación externa (8 subagentes, comparadores de carreras reales en Chile/Latam/EE.UU./Reino Unido/Europa/Australia, ~55 hallazgos). 19 ideas priorizables en 6 direcciones temáticas, sección aspiracional (requiere fuente externa nueva) y 6 tandas de ejecución sugeridas. Sin dirección final elegida — se decide en T26b. |
| **26b** | Discutir hallazgos de T26 y decidir dirección | ninguno (sesión de conversación, sin código) | En cola — ver `TAREA_26b_PROMPT.md`. Objetivo: revisar `BRAINSTORM_T26.md` con Diego y salir con una tanda (o combinación) priorizada, lista para su propio `TAREA_N_PROMPT.md` de implementación. |
| **26c** | Compartir comparación: reubicar el botón | `index_v2.html`, `instituciones_v2.html`, `nem.html`, `sw.js` | ✅ 2026-08-16 — Surgió fuera de la cola de T26/T26b (pedido directo de Diego). Reemplaza la franja verde de ancho completo (T16) por una píldora "🔗 Compartir" en línea con el texto de ayuda del selector — misma ubicación en las 3 herramientas, agregada en `nem.html` (no la tenía desde T24). De paso: si un link compartido referencia un slug/id que ya no existe en el dataset actual, ahora se avisa ("N ítems de este enlace ya no están disponibles") en vez de omitirlo en silencio. SW → `carreraclara-v12`. |
| **27** | Ficha de institución (pantalla nueva) | nuevo archivo | Post-MVP — ya redactado (`TAREA_27_PROMPT.md`), en cola desde antes de T26; T26b puede confirmar su prioridad o reordenarla. |
